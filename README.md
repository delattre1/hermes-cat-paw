...
For the Omarchy or Windows demo, build and run the fork from source on the
computer you want Hermes to control:

- **Windows:** clone [`kumanaya/cat-paw-latch`](https://github.com/kumanaya/cat-paw-latch) and run `just app`.
- **Linux / Omarchy:** same — clone the repo and run `just app`; see its README for the prerequisites.
