...
# Recon pack is a runtime clone, not baked into the image. Failure here does
# not undo the line; re-run scripts/install-skills.sh.
if ! "$ROOT/scripts/install-skills.sh"; then
  echo "install.sh: recon skill pack did not land. Re-run scripts/install-skills.sh." >&2
fi
